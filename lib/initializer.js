...
Ember.onLoad('Ember.Application', function(Application) {
  ...
  if (Ep.DebugAdapter) {
    Application.initializer({
      name: "dataAdapter",
      initialize: function(container, application) {
        console.log("registering Ep.DebugAdapter for ember-extension dev tool");
        application.register('dataAdapter:main', Ep.DebugAdapter);
      }
    });
  }

  // Inject the store into the debugAdapter
  // Store is backed by session in EPF
  // This code makes sure that the debugAdapter has access to the session
  Application.initializer({
    name: "injectStore",
    initialize: function(container, application) {
      console.log("injecting session into store for debugAdapter used by ember-extension dev tool");
      application.inject('dataAdapter', 'store', 'session:main');
    }
  });
